...
var menusRoute = "/menu";
var myITNRoute = "/postsbycategories"
...
var myITNURL = rootURL + APIPath + namespaceITN + myITNRoute;
...
{
    SC.initialize({
        client_id: '29fd70e9af8fd1e6345087ec391dc3fe'
    });

    SC.oEmbed('https://soundcloud.com/irish-tech-news', {element: document.getElementById('podcastWrapper') });

}

function refreshPage(catID, catName)
{
    if(catID === -1 )           // refresh the home page
    {
        getHomePage(); 
    }
    else if(catID === -2 )      // refresh the home page
    {
        getMyITNPage();
    }
    else                        // refresh a category page
    {
...
function getMyITNPage()
{
    ...
    $(ITNCats).each(function(index, oneCat)
    {
        ...
        {
            faveIDs.push(oneCat.catID)
        }
    });

    GLOBAL_paramData.arrayofids = faveIDs;

    $.when(getJSON(myITNURL, GLOBAL_paramData)).done(function(data)
    {
        $('#title-header').html("Irish Tech News"); // update title-header to active category clicked
        $('#title-header').data('catid',-2);        // set a flag to tell us we are on the My ITN page
...
function getPostsByCategory(categoryID, categoryName) 
{
    ...
    $.when(getJSON(postsURL, GLOBAL_paramData)).done(function(data)
    {
        //Is this a trending Category
        var foundIndex;
        var isTrending =  ITNTrendingCats.some(function(one, index) {

            if(one.trendingCatName === categoryName){
                foundIndex = index;
                return true;
            }
        }); 
        ...
        if(isTrending)
        {
            ...
            $('.carousel').flickity( 'selectCell',  ITNTrendingCats[foundIndex].linkedCarouselID);
        }
        else
        {
...
function displayPost(data) 
{
    ...
    {  
        $(data).each(function(i, item) 
        {
            ...
            listHTML += '<div class="boxInner textOverlay"><img src="' + item.image_URLs[0].fullImage + '" />';
...
function addTrendingToMenu()
{
    ...
    $('.trendingCats').unbind().click(function(e) 
    {
        ...
        getPostsByCategory(this.id,  $(this).data('catname'), false); 
    });
}
...
function addTrendingToCarousel()
{
    ...
    var cellElements = $('.carousel').flickity('getCellElements').length;
    ...
    $(ITNTrendingCats).each(function(index, oneJSONCat) 
    {
        ...
        oneJSONCat.linkedCarouselID = cellElements;
        cellElements++;
    });
    
    localStorage.setItem("trendingCats", JSON.stringify(ITNTrendingCats));
    ...
    $('.carousel').on( 'staticClick.flickity', function( event, pointer, cellElement, cellIndex ) 
    {
        if ( typeof cellIndex == 'number' ) 
        {
            ...
            else if($(cellElement).data('catname') === "myITN")
            {
                getMyITNPage();
            }
            else
